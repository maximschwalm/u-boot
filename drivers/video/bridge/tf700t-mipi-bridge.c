...
struct tc358768_priv {
	struct gpio_desc ldo;
	struct gpio_desc vdd;
	struct gpio_desc vddio;
...
static int tc358768_enable(struct udevice *dev)
{
	...
	ret = gpio_request_by_name(dev, "ldo-gpios", 0,
				   &uc_priv->ldo, GPIOD_IS_OUT);
	if (ret) {
		debug("%s: Could not decode ldo-gpios (%d)\n", __func__, ret);
		if (ret != -ENOENT)
			return ret;
	}

	ret = gpio_request_by_name(dev, "vdd-gpios", 0,
				   &uc_priv->vdd, GPIOD_IS_OUT);
	if (ret) {
		debug("%s: Could not decode vdd-gpios (%d)\n", __func__, ret);
		if (ret != -ENOENT)
			return ret;
	}

	ret = gpio_request_by_name(dev, "vddio-gpios", 0,
				   &uc_priv->vddio, GPIOD_IS_OUT);
	if (ret) {
		debug("%s: Could not decode vddio-gpios (%d)\n", __func__, ret);
		if (ret != -ENOENT)
			return ret;
	}

	ret = gpio_request_by_name(dev, "lvds-gpios", 0,
				   &uc_priv->lvds, GPIOD_IS_OUT);
	if (ret) {
		debug("%s: Could not decode lvds-gpios (%d)\n", __func__, ret);
		if (ret != -ENOENT)
			return ret;
	}

	ret = gpio_request_by_name(dev, "power-gpios", 0,
				   &uc_priv->power, GPIOD_IS_OUT);
	if (ret) {
		debug("%s: Could not decode power-gpios (%d)\n", __func__, ret);
		if (ret != -ENOENT)
			return ret;
	}

	/* Enable sequence */
	ret = dm_gpio_set_value(&uc_priv->ldo, true);
	if (ret)
		return ret;

	mdelay(20);

	ret = dm_gpio_set_value(&uc_priv->vdd, true);
	if (ret)
		return ret;

	ret = dm_gpio_set_value(&uc_priv->vddio, true);
	if (ret)
		return ret;

	mdelay(10);

	ret = dm_gpio_set_value(&uc_priv->lvds, true);
	if (ret)
		return ret;

	ret = dm_gpio_set_value(&uc_priv->power, true);
	if (ret)
		return ret;
...
U_BOOT_DRIVER(tf700t_mipi_bridge) = {
	.name	= "tf700t_mipi_bridge",
